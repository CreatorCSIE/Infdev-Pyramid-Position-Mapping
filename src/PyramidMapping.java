...
public class PyramidMapping {
    ...
    private static void handleSearch(JTextField xField, JTextField zField,
    		...
    		String xInput = xField.getText().replace(",", "").trim();
    			...
    			try {
    				...
    				// 范围验证
    				String rangeError = checkCoordinateRange(targetX, targetZ);
    				if (rangeError != null) {
    ...
    //坐标范围验证（返回具体错误描述）
    private static String checkCoordinateRange(double x, double z) {
    	if (x < 0 || z < 0) {
    		return "坐标不能为负数";
    	}
    	if (x > 33554432 || z > 33554432) {
    		return String.format("坐标不能超过 %,d", 33554432);
    	}
    	return null;
    }
